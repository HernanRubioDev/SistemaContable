const AccountSelect = ({accounts, accountsForm, handleChange})=>{
  ...
  <form className="d-flex p-0 gap-3">
    <div className="d-flex flex-column">
      ...
      <select onChange={(e)=>handleChange(e)} className="form-select" id="positivesResultes" name="positiveResult" value={accountsForm.positiveResult}>
    ...
    <div className="d-flex flex-column">
      ...
      <select onChange={(e)=>handleChange(e)} className="form-select" id="negativesResults" name="negativeResult" value={accountsForm.negativeResult}>
        <option value="" defaultChecked unselectable="on">Seleccione una cuenta</option>
        {accounts.map(acc => acc.account_type === "R-" && <option key={acc.id_account}>{acc.name}</option>)}
      </select>
    </div>
